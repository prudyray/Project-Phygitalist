"""
Management command: update_index_categories

Full reindex of all browsable categories into Manticore.
"""

import time
from django.core.management.base import BaseCommand
from shop.apps.search.indexer import CategoryManticoreIndex, _chunked
from shop.apps.search import defaults


class Command(BaseCommand):
    help = "Rebuild the Manticore categories search index."
        ...
        for chunk in _chunked(qs.iterator(chunk_size=chunk_size), chunk_size):
            ...
            for obj in chunk:
                try:
                    ...
                    self.stderr.write(
                        f"Failed to build doc for category id={obj.id}: {exc}"
                    )
            if pairs:
